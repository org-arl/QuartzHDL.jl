...
# it works hardest on is one the design allows many cycles for.
function _fdc(io::IO, T::Type{<:QuartzModule}, b::Board, overconstrain::Float64)
  ...
  for net in sort(collect(keys(rates)))
    ...
    println(io, "create_clock -name {$(_pinname(T, net))} -period $(round(period; digits=3)) [$where {$(_pinname(T, net))}]")
  end
  for M in _allmodules(T), m in _allmulticycles(M)
...
function _fdcmulticyclestr(T::Type, M::Type, m::MultiCycle)
  join(("set_multicycle_path $(m.cycles) -from [get_cells {$(_prefix(path))$(m.from)*}] -to [get_cells {$(_prefix(path))$(m.to)*}]"
