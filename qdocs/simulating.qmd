...
The capture — what `@run` returned above, or `capture(sim)` at any time — holds every watched net's history, as values: a pin asserted low reads `true` while asserted, as the design sees it, and [plots and VCDs](visualizing.qmd) show the wires. Query it by name or path, and by time:
