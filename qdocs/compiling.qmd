...
**Writing a design out as Verilog**
...
`quartz timing design.jl --top Top` prints the [timing report](timing.qmd) for a module and does not compile it. With `--budget`, the exit status shows whether the design is within the budget.
...
build/rev2/
  ...
  Rev2.fdc           the same clocks and multicycle paths, in the form Synplify reads
  Top.ldf            the project file: device, top, sources, strategy
  Top.sty            Diamond's default strategy, with synthesis retiming off and timing-driven map
...
Map and place-and-route read the `.lpf`. Synplify reads the `.fdc`, and without one it times every path against a default clock and does not know about the multicycle paths, so it may spend its effort on a path the design allows many cycles for. The two files are written from the same declarations, each in the spelling its reader expects: a clock made by a black box with a netlist is named to Synplify by the pin that drives it, `pll1.CLKOP`.
...
A black box needs its netlist in the workspace. Give the files with `vendor = ["vendor/PLL48.v", ...]` and they are copied into `src/` and listed in the project; a black box with no netlist among them is still listed, as `src/<Name>.v`, with a warning saying where to put the file, unless it is declared `primitive=true`, a module Diamond knows by itself. `implementation = "impl"` names Diamond's implementation and its directory.

The strategy turns synthesis retiming off, which [multicycle paths](multicycle.qmd) need, and turns timing-driven mapping on. Timing-driven packing and node replication stay off. On the one design they were measured on, over twenty placement seeds, packing raised the median clock speed slightly but made one seed in twenty fail, and replication changed nothing. `pack = true` and `replicate = true` turn them on for a design where a measurement shows a gain. Two options help with [measuring timing](timing.qmd). `overconstrain = 1.2` constrains every clock at 1.2 times its real rate, so that a build shows how much margin it has. `paths = 100` sets how many paths the timing reports list for each constraint. `LPF(board; overconstrain = 1.2)` writes only the constraint file with the same scaling.
