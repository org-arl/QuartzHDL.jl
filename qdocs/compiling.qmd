...
Two things you may be used to doing by hand are done for you and are worth knowing about. Every port asserted low is bridged at the boundary (`assign rst = !rst_ni;`) and used asserted inside, so the module body never mentions `_n`. And registers with a reset default get no initializer, so the synthesiser keeps the flip-flops' enable and clear pins free; only `static` fields carry `initial` values.
:::

## The command line

The package doubles as an app. `julia -m QuartzHDL design.jl` compiles the `@quartz` modules in a Julia file to Verilog without a driver script:
...
`Pkg.Apps.develop(path = "...")` (Julia 1.12 or later) installs a `quartz` executable, so the `julia -m QuartzHDL` prefix becomes just `quartz`.
