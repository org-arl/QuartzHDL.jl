...
Map and place-and-route read the `.lpf`. Synplify reads the `.fdc`, and without one it times every path against a default clock and does not know about the multicycle paths, so it may spend its effort on a path the design allows many cycles for. The two files are written from the same declarations.
