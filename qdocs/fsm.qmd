...
`step` is a `Step` register the struct declares, and the only register the sequence uses: it starts at `START`, is sixteen bits in the Julia model, and is emitted as wide as the steps need. A `Bits{N}` field serves as well, when it is wide enough. The statements between dividers are one step: one cycle, all writes in parallel, then on to the next.
