...
`Writer` shows every divider once; here is the same construct with a purpose. A byte goes out as a start bit, eight data bits, a parity bit and a stop bit, each held for one bit time — and the bit time is a [`Timeout`](timekeeping.qmd), reloaded at every step, so the sequence waits on `expired` rather than counting:
