...
A `Multicycle{K,T}` is a wire allowed K cycles to settle, on the promise that the registers it reads hold still that long. The compiler writes the constraint, the settle counter and the simulator's check from the same traced logic.
...
A path that is not a wire's — a slow transfer between two registers, say — can still be declared for the constraint file, once per module:
...
- The inputs hold still for K cycles at a time: a `Multicycle`. It costs nothing but a counter, and the promise is checked in simulation and stated to the tools.
