...
For a design with several clocks, stepping by hand means advancing each clock in the right ratio. A [`Bench` or `Simulation`](benches.qmd) takes a clock plan with real rates and advances every clock in the right order, so a slow-clock block sees exactly what it would on the board.
...
[Pipelines](pipelines.qmd): arithmetic that takes several cycles, cut into stages by the compiler.
